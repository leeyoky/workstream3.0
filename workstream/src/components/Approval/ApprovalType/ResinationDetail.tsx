...
const ResinationDetail: React.FC<ResinationDetailProps> = ({ setTemp }) => {
  ...
  const {
    ...
    homePhoneChangeHandler: HPhoneHandler,
  } = usePhoneValidation();
  ...
  const initializeData = () => {
    if (data) {
      setRetireDate(
        data.resignation.resignationDate ? new Date(data.resignation.resignationDate) : null,
      );
  ...
  useEffect(() => {
    setSSNFront(validationSSNFront);
  }, [validationSSNFront]);
  useEffect(() => {
    setSSNBack(validationSSNBack);
  }, [validationSSNBack]);
  useEffect(() => {
    setMobilePhone(MBPhone);
  }, [MBPhone]);
  useEffect(() => {
    setHomePhone(HPhone);
  }, [HPhone]);
          ...
          <table className={classes['header-info']}>
            <tbody>
              <tr>
                ...
                <td colSpan={2}>{data?.resignation.rankNm}</td>
              </tr>
              <tr>
                <th>성 명</th>
                ...
                {isEdit ? (
                  ...
                  <td colSpan={2}>{data?.resignation.identityNo}</td>
                )}
              </tr>
              <tr>
                <th>입사 일자</th>
                ...
                {isEdit ? (
                  ...
                  <td colSpan={2}>{data?.resignation.resignationDate}</td>
                )}
              </tr>
              <tr>
                ...
                {isEdit ? (
                  ...
                  <td>{data?.resignation.homeContact}</td>
                )}
              </tr>
              <tr>
                <th>휴대폰</th>
                {isEdit ? (
                  ...
                  <td>{data?.resignation.mobileContact}</td>
                )}
              </tr>
              <tr>
                <th>퇴직 사유</th>
            ...
            <tbody>
              <td>
                <div className={classes['text-wrapper']}>
                  <div>
                    ...
                    <p className={classes['first-paragraph']}>
                      ① 상기자 본인은 후임자에게 업무인계를 성실히 이행하겠습니다.
                    </p>
                    <p>
                      ② 상기자 본인은 부정경쟁방지법 제11조 및 12조의 규정과 회사의 비밀 관리 규정을
                      준수하여, 근무 중 인지한 경영, 재정, 영업, 기술, 마케팅 등
                      <br />
                      &emsp; 회사기밀에 해당하는 내용을 타사 및 기타 방법으로 절대 이용치 않을
                      것이며, 경쟁사에 사전 허가 없이 누설 혹은 유출 시 회사에 발생한
                      <br />
                      &emsp; 유,무형의 손실에 대해 전적으로 민형사상 책임을 지겠습니다.
                    </p>
                  ...
                  <div className={classes['second-paragraph']}>
                    ...
                    <p className={classes['first-paragraph']}>
                      ① 현업 부서장은 부서원이 사직할 경우 업무적인 인수인계 부분을 최종적으로
                      확인한다. (부서장이 퇴사할 경우 차 하위직원이 확인){' '}
                    </p>
                    <p>② 현업 부서장은 사직에 관련된 제반 첨부서류 내용을 최종적으로 확인한다.</p>
                    <p>
                      ③ 현업 부서장은 사직원 결재란에 최종 결재하면서 사직자와 회사 사이에 발생한
                      채권, 채무 관계를 마무리하여야 하며, 이러한 관리를 소홀히
                      <br />
                      &emsp; 하였을 때는 업무태만으로 간주하여 민, 형사상 책임을 진다.
                    </p>
                  </div>
                </div>
                ...
                <div className={classes['attachment-warpper']}>
                  ...
                  <div>
                    <p>
                      <span className={classes['attachment-item']}>
                        1. 업무인수인계 확인서 1부.
                      </span>
                    </p>
                    <p>
                      <span>2. 업무인수인계 세부내역서 1부.</span>
                    </p>
                    <p>
                      <span>3. 퇴직서약서 1부.</span>
                    </p>
                  </div>
                </div>
              </td>
